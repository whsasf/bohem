...
#for ((i=1;i<=$count;i++ ))
#do
	length=`wc -c virus-code-normal-texts-messagebody-IMAP-base64.txt |awk  '{print $1}'`
	echo -e "\n"
	echo "Message$i body length = $length "
	echo "Message$i is:"
	DATA=`cat virus-code-normal-texts-messagebody-IMAP-base64.txt`
  echo -e "$DATA"
	echo -e "\n\n!!!!!!!!!!!!!!!!!!!!!!!!!!!!Appending message$i!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
...
cat summary.log |grep  "FETCH (FIRSTLINE ("  |grep -v UID  >fetch-target.txt 
unix2dos  fetch-target.txt  #LF to CRLF

diff fetch-target.txt  fetch-template-base64-base64.txt >diff-temp.log
...
diff uidfetch-target.txt  uidfetch-template-base64-base64.txt >diff-temp.log
