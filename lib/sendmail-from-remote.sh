...
usage()
{
	cat <<- EOF
		  ...
		  sendmail-from-remote.sh remote-server mail-server[:port] sender
...
ssh -o ConnectTimeout=10 -o StrictHostKeyChecking=no -o BatchMode=yes $remote_server bin/sendmail-from.pl $mail_server $mail_server_port $sender
