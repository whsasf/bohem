...
# commands to some port - in this case the IMAP one. 
# 
# For usage (example) see the imapmail.pl script
#
...
sub new
{
    ...
    bless $obj, 'imap';                    #return the reference to this object
    return $obj;
}
...
sub DESTROY
{
    ...
    imap_disconnect() if ($obj->{connected} == 1);
}
...
sub get_imap_response
{
    ...
    *F = $obj->{socket};
    ...
    while ($line = <F>)                                         #read from the socket
    {
        ...
        $obj->showmsg("Received: $line");
        print ("$line"); #if ($line =~ /^\* /);
        last if ( ($line =~ /^$tag OK.*/) || ($line =~ /^$tag NO.*/) );
    }
    return $buffer;
}
