#!/usr/bin/perl
#
# Usage: imapmail-delay.pl <host> <port> <user> <pass> <select-delay>,<fetch-delay> <folder> [<message>]
# 		Mail data is output to stdout.
#
# Sleep for specified duration prior to select and prior to fetch
#  NOTE that the delay prior to SELECT probably isn't useful however, delaying prior to fetch
#         allows you to lock a mailbox and then detect message delivery during the locked period.
...
@delays = split(/,/, $DELAY);
$select_delay=@delays[0];
$fetch_delay=@delays[1];

my($session,$telnetResult);

print "imapmail-delay.pl: host[$HOST] port[$PORT] user[$USER] pass[$PASS] select_delay[$select_delay] fetch_delay[$fetch_delay] folder[$FOLDER] message[$MSG]\n";
...
sleep "$select_delay" if ($select_delay);
...
if ($fetch_delay) {
	sleep "$fetch_delay";
	$rc = $session->sendCommand ("xx NOOP");
	$x = $session->get_imap_response("xx");
	if ($x !~ /.*(\n|^)xx OK.*/) {
		printf "Didn't get OK for NOOP\n";
		exit 17
	}
}
