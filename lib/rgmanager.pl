#!/usr/bin/perl
#
# Usage: rgmanager.pl <host> <port> <command> -- no login, execute command directly
#    or  rgmanager.pl <host> <port> <passord> <command> -- login as administrator and execute command
#    or  rgmanager.pl <host> <port> <user> <passord> <command>  -- login as specfied user and execute command
#        
...
# Open a connection
# Read the banner
# Send login if user and password are given
# Read OK
# Send command (adding "aa" tag to command before sending)
...
sub login
{
	...
	if ( defined $PASSWORD )
	{
		$ret = $obj->sendCommand("login $USER $PASSWORD"); 
		$obj->{check_if_connected} = 1;
		if ( $ret == 0 )
		{
			print("Login Failed...\n");
...
sub get_response
{
    ...
    *F = $obj->{socket};
    ...
    while ($line = <F>)                                         #read from the socket
    {
        ...
        print ("$line") if ($line =~ /^\* /);
        last if ( ($line =~ /^aa OK.*/) || ($line =~ /^aa NO.*/) || ($line =~ /^\+ */) );
    }
    return $buffer;
}
