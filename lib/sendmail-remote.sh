...
usage()
{
	cat <<- EOF
		  ...
		  sendmail-remote.sh remote-server[:port] mail-server sender recipient
...
ssh -o ConnectTimeout=10 -o StrictHostKeyChecking=no -o BatchMode=yes $remote_server bin/sendmail.pl $local_ip_args $mail_server $mail_server_port $sender $*
