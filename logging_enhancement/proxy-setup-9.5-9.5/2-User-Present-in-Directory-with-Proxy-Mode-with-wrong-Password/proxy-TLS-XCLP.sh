...
targett2="fromhost=10.49.58.127:|fromhost=\[10.49.58.127\]:fromport="
targett3="fromhost=10.37.2.215:fromport=|fromhost=\[10.37.2.215\]:fromport="
...
ssh root@${IMAPHost1}  			 ' > /opt/imail2/log/imapserv.log'
ssh root@${IMAPHost2}  			 ' > /opt/imail2/log/imapserv.log'
...
ssh root@${POPHost1}  ' > /opt/imail2/log/popserv.log'
ssh root@${POPHost2}  ' > /opt/imail2/log/popserv.log'
...
#SMTP relay transparent testing
echo -e "\n\n###################################### SMTP proxy transparent testing ###########################################"

#clear all mta.log in MX9.5-1 and MX9.5-2
ssh root@${SMTPHost1}  ' > /opt/imail2/log/mta.log'
ssh root@${SMTPHost2}  ' > /opt/imail2/log/mta.log'
...
then
  ...
  echo -ne "\n\033[32m#####Logging enhancement for SMTP Relay transparent is ok!!##### \033[0m\n\n" >>summary-smtp.log
  echo -ne "\n\033[32m#####Logging enhancement for SMTP Relay transparent is ok!!##### \033[0m\n\n" >>outcome.log
  echo -ne "\n\033[32m#####Logging enhancement for SMTP proxy transparent is ok!!##### \033[0m\n\n" >>../all-summary.log
else
  echo -ne "\n\033[31m#####Logging enhancement for SMTP Relay transparent not ok!!##### \033[0m\n\n"
  echo -ne "\n\033[31m#####Logging enhancement for SMTP Relay transparent not ok!!##### \033[0m\n\n" >>summary-smtp.log
  echo -ne "\n\033[31m#####Logging enhancement for SMTP Relay transparent not ok!!##### \033[0m\n\n" >>outcome.log
  echo -ne "\n\033[31m#####Logging enhancement for SMTP proxy transparent not ok!!##### \033[0m\n\n" >>../all-summary.log
fi



#restart mta on dest server
ssh root@${FEPHost2} "su - ${imailuser} -c \"/opt/imail2/lib/imservctrl killStart mta\""

sleep 6

#SMTP relay nontransparent testing
echo -e "\n\n###################################### SMTP proxy nontransparent testing ###########################################"
...
#Prepare the summary-smtp.log
echo -e  "The smtp operations logs telneting to MX9.5-1:\n\n" >summary-smtp2.log
cat smtp-temp.log >>summary-smtp2.log
echo -e "\n\n" >>summary-smtp2.log
echo -e "\n\nSMTPserv.log from MX9.5-1:\n"
cat mta1.log
echo -e "\n\nSMTPserv.log from MX9.5-2:\n"
cat mta2.log
echo -e "\n\nSMTPserv.log from MX9.5-1:\n" >>summary-smtp2.log
cat mta1.log >>summary-smtp2.log
echo -e "\n\nSMTPserv.log from MX9.5-2:\n" >>summary-smtp2.log
cat mta2.log >>summary-smtp2.log
#analyze    

c1=`grep -E "$targett3" mta1.log|wc -l`
c2=`grep -E "$targett3" mta2.log|wc -l`
let cc1=c1+c2
tlsflagct=`grep $tlsflag mta2.log|wc -l`
echo cc1=$cc1
if (( $cc1 == 13 )) && (( $tlsflagct == 1 ))
then
  echo -ne "\n\033[32m#####Logging enhancement for SMTP Relay nontransparent is ok!!##### \033[0m\n\n"
  echo -ne "\n\033[32m#####Logging enhancement for SMTP Relay nontransparent is ok!!##### \033[0m\n\n" >>summary-smtp2.log
  echo -ne "\n\033[32m#####Logging enhancement for SMTP Relay nontransparent is ok!!##### \033[0m\n\n" >>outcome.log
  echo -ne "\n\033[32m#####Logging enhancement for SMTP proxy nontransparent is ok!!##### \033[0m\n\n" >>../all-summary2.log
else
  echo -ne "\n\033[31m#####Logging enhancement for SMTP Relay nontransparent not ok!!##### \033[0m\n\n"
  echo -ne "\n\033[31m#####Logging enhancement for SMTP Relay nontransparent not ok!!##### \033[0m\n\n" >>summary-smtp2.log
  echo -ne "\n\033[31m#####Logging enhancement for SMTP Relay nontransparent not ok!!##### \033[0m\n\n" >>outcome.log
  echo -ne "\n\033[31m#####Logging enhancement for SMTP proxy nontransparent not ok!!##### \033[0m\n\n" >>../all-summary.log
fi
